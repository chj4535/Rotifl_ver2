...
module.exports = function (param) {
    ...
    group.get("/:groupid", (req, res) => {
        var html = "'get'/" +  param['userid'] + "/group/" +  param['groupid'] + " 페이지 입니다.";
        res.writeHead(200, {'Content-Type': 'text/plain; charset=utf-8'});//한글 깨짐 방지용
        res.end(html);
    });

    group.put("/:groupid", (req, res) => {
        var html = "'put'/" + param['userid'] + "/group/" + param['groupid'] + " 페이지 입니다.";
        res.writeHead(200, {'Content-Type': 'text/plain; charset=utf-8'});//한글 깨짐 방지용
        res.end(html);
    });
    ...
    group.delete("/:groupid", (req, res) => {//그룹 삭제 ( 자기가 나가는거임)
        ...
        MongoClient.connect('mongodb://localhost:27017/',  { useNewUrlParser: true }, function (error, client) {
            if (error) console.log(error);
                ...
                const db = client.db(dbName);
                db.collection('groups').remove({groupuserid:param['userid'], groupid:Number(param['groupid'])})
            }
        });
    });
    ...
    group.get("/:groupid/user", (req, res) => {
        var html = "'get'/" + param['userid'] + "/group/" + param['groupid'] + "/user 페이지 입니다.";
        res.writeHead(200, {'Content-Type': 'text/plain; charset=utf-8'});//한글 깨짐 방지용
        res.end(html);
    });
